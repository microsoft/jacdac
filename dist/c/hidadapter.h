...
#ifndef _JACDAC_SPEC_HID_ADAPTER_H
#define _JACDAC_SPEC_HID_ADAPTER_H 1

#define JD_SERVICE_CLASS_HID_ADAPTER  0x1e5758b5

/**
 * Read-write uint8_t. The number of configurations stored on the server.
 */
#define JD_HID_ADAPTER_REG_NUM_CONFIGURATIONS 0x80

/**
 * Read-write uint8_t. The current configuration the server is using.
 */
#define JD_HID_ADAPTER_REG_CURRENT_CONFIGURATION 0x81

/**
 * Retrieves a configuration stored on the server. If the configuration does not exist, an empty report will be returned
 */
#define JD_HID_ADAPTER_CMD_GET_CONFIGURATION 0x80
typedef struct jd_hid_adapter_get_configuration {
    uint8_t results[12];  // pipe
    uint8_t configuration_number;
} jd_hid_adapter_get_configuration_t;


/**
 * Retrieves a configuration stored on the server. If the configuration does not exist, an empty report will be returned
 */
typedef struct jd_hid_adapter_configuration {
    uint8_t configuration_number;
    ...
    uint16_t selector;
    uint16_t modifiers;
} jd_hid_adapter_configuration_t;


/**
 * Stores the given binding on the server. If a binding exists at this index, the new binding will replace it.
 */
#define JD_HID_ADAPTER_CMD_SET_BINDING 0x82
typedef struct jd_hid_adapter_set_binding {
    uint8_t configuration_number;
    uint8_t binding_index;
    uint16_t padding;  // u8[2]
    uint64_t device_id;
