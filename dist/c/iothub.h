 ...
 */
#define JD_IOT_HUB_PIPE_SUB_PROPERTIES 0x1
typedef struct jd_iot_hub_sub_properties {
    uint32_t identifier; // 0x1
    char property_name[0];  // string0
    // char property_value[0];  // string0
} jd_iot_hub_sub_properties_t;


/** Argument: body bytes. For every message, the body is sent once, using this report. */

/** Argument: twin_result pipe (bytes). Ask for current device digital twin. */
#define JD_IOT_HUB_CMD_GET_TWIN 0x85

/**
 * Argument: json bytes. The JSON-encoded twin; note that there can be several packets that need to be concatenated.
...
/** Argument: json bytes. Every back-end change is sent as one or more reports like this. */

/** No args. This is send after last `twin_update_json` packet for a given update. */
#define JD_IOT_HUB_PIPE_TWIN_UPDATE_END 0x1
typedef struct jd_iot_hub_twin_update_end {
    uint32_t identifier; // 0x1
} jd_iot_hub_twin_update_end_t;
...
#define JD_IOT_HUB_REG_IS_CONNECTED 0x180
