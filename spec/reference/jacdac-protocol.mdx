...
* **JACDAC packets** and their logical structure; 
...
struct _jd_packet_t {
    ...
    uint8_t service_number;       // the index into the device's advertised list of services
...
This means that the interpretation of the `service_number` field is relative to the destination device for command packets
and is relative to the source device for report packets. Read more about command and report packets in the next section.
...
In constrast, a service number (`service_number` field in the JACDAC packet) is an index into the list of services advertised by a device. The control service has both service class 0 and service number 0.
...
The control service is the heart of the JACDAC protocol, with each device running its own copy of the service. The service class of the control service is reserved to be zero (0).
...
### Device state and service numbers

A device will generally maintain device-specific state for each of the services that it supports. This state should be kept in a list, where each list entry contains the service class and the device-specific state for that service. When a device advertises its services they are ordered in the same sequence as in the list.  This allows clients of the device to refer to one of its services by list index (`service_number`, an 8 bit quantity) instead of service class (a 32 bit quantity).
...
### Device presence/absence and advertisement updates
