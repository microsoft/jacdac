...
A Jacdac device is any device that exposes the [Jacdac PCB edge connector](https://github.com/microsoft/jacdac-ddk/tree/main/design#pcb-edge-connector-in-detail)
...
2. Use a hardware random number generator in the MCU to generate a random 64 bit number during device commissioning - for example at first boot. This could use sensor values to create further entropy. Once generated it must be stored in non-volatile flash or EEPROM memory for future use.
...
Jacdac devices with a [Jacdac PCB edge connector](https://github.com/microsoft/jacdac-ddk/tree/main/design#pcb-edge-connector-in-detail)
...
[Read more about the network interface layer.](#network-interface-layer)
...
[Read more about the hardware layers.](#hardware-layers)
...
For more information, see [the single wire serial protocol](../network-interface-layer/sws).
