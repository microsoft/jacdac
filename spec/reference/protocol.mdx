...
Devices can support reliable actions, which use the same encoding as events, but are commands, not reports.
The `service_command` field is subdivided as follows:

| Bits | Function 
|------|----------------------
| 15   | Always set for reliable actions
| 14:8 | Sequential counter
| 7:0  | Action code

Only action codes in range `0x00-0xff` are supported.
Just as for events, the sequential counter is per-device (but separate from the event sequential counter).
Reliable actions should be processed exactly like events - if a command comes out of order, it should be ignored.
The repetition scheme is the same as for events.

Unlike events, reliable actions can be sent with ACK requested.
If an ACK is received, further re-transmissions of action can be skipped.

Support for reliable actions is announced as a flag in the announce packet.
...
When a device cannot handle a command, it can send an error response.
