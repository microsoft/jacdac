...
Multiple invocations of actions that specify a pipe in the arguments are implicitly ignored
(since, it's not possible to open the same pipe multiple times simultaneously).
Thus, they do not require the `unique` attribute.

For non-idempotent actions, a pipe can be opened with a command on the control service.
Commands (or any other packets) can then be sent over that pipe.
