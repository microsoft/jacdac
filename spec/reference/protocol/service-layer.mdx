...
The client should ignore the repetitions it recives.
...
If an event comes with an unexpected number, the client should wait with handling it
for at least 100ms, until the preceding (repetitions of) events arrive.
Then the client should handle events in the intended order.
