...
- **awaitEvent**(*event*,*expr*): wait for the *event* to arrive (ignoring other events) and then check *expr* (which may refer to arguments of *event*)
- **nextEvent**(*event*,*expr*): next event should be *event* (fail if other events), and then check *expr* (which may refer to arguments of *event*)
