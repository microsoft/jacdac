...
Each register or command has a number associated with it and the contents of a JACDAC packet references this number via the `service_api` field. These numbers are referred to as "register codes" or "command codes" respectively. Both register and command codes are 12-bits in length; the `service_api` field itself is 16-bits in length.

The top nibble of the `service_api` is used to specify the type of operation:
...
Command codes have the following defined ranges:

| Command code range | Purpose 	|
|--------|------------	|
| `0x000-0x07f` | Command codes common to all services	|
| `0x080-0xeff` | Command codes specific to a service	|
| `0xf00-0xfff` | Reserved	|

When creating a new service, new commands must begin at 0x080. Note that all commands have the top nibble set to zero.
