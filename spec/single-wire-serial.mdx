...
Each service has an associated application programming interface (API). Each API can fall into one of two categories:

* __Registers__: Properties that represent the state of a service. Registers can be inspected and modified, depending on what permissions are configured.
* __Commands__: Actions that can be performed on a service. Ultimately, commands can also affect the state of a register.
...
All services are expected to respond to application common command codes.

### Register codes

Registers can be only be accessed by a read or write operation. To perform either operation the register code is combined with the operation nibble listed above (i.e. `0x1` for a read, `0x2` for a write):
