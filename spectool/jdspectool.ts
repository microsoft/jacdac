        ...
        public ${rtype} ${capitalize(camelize(rname))}
        {
            get
            {
                ${
                    client
                        ? `// TODO: implement client register
                   throw NotImplementedException("client register not implemented");`
                        : `return ${fetchReg};`
                }
            }${
                kind === "rw"
    ...
    .map(command => {
            ...
            .join(", ")})
        {
            ...
            throw new NotImplementException("client command not implemented");`
                : `this.SendCmdPacked(${cmd}, ${pack}, new object[] { ${fnames.join(", ")} });`
        }
        }
`
    })
...
function processSpec(dn: string) {
    ...
    for (const fn of files) {
        ...
        for (const n of Object.keys(cnv)) {
            ...
            if (n == "cs") {
                fnn = path.join("Constants", capitalize(camelize(fnn.replace(/^_/, ''))) + "Constants");
